export async function POST(request: Request){
    const beehiivURL = process.env.NODE_ENV === 'production' ? "https://api/beehiiv.com/v2"
    ...
    const rest = await fetch(`${beehiivURL}/publications/${process.env.BEEHIV_PUBLICATION_ID}/subscriptions`, {
        method: 'POST',
        headers: {
